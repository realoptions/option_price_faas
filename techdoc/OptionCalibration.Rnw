...
options(digits=4)
base='./'
#base='~/Documents/code/finside/option_price_faas/techdoc/'
@

<<fig=T, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE>>=
...
df <- data.frame(Type = c("Actual", "Estimate"), eta=c(actual$eta_v, estimates$eta_v), speed=c(actual$speed, estimates$speed), sigma=c(actual$sigma, estimates$sigma), rho=c(actual$rho, estimates$rho))
strCaption <- paste0("Heston Comparison")
